    ...
    <div class="bg-article rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <div class="flex">
            <input 
              ...
              :value="articleUrl" 
              readonly 
              class="flex-1 block w-full rounded-l-md border-accent-bg shadow-sm focus:border-primary focus:ring-primary sm:text-sm px-3 py-2 bg-bg-muted text-fg-muted"
            >
            <button 
              @click="copyLink" 
              class="inline-flex items-center px-3 py-2 border border-l-0 border-accent-bg rounded-r-md bg-bg hover:bg-bg-alt text-sm font-medium text-primary focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"
            >
              <Icon :name="copyIcon" class="w-4 h-4 mr-1.5" />
              {{ copyButtonText }}
            </button>
          </div>
        ...
        <div class="flex space-x-2">
           <a :href="`https://www.linkedin.com/sharing/share-offsite/?url=${encodedUrl}`" target="_blank" rel="noopener noreferrer" class="social-button bg-[#0077b5] hover:bg-[#005e90]">
              <Icon name="mdi:linkedin" class="w-5 h-5" />
           </a>
           <a :href="`https://twitter.com/intent/tweet?url=${encodedUrl}&text=${encodedTitle}`" target="_blank" rel="noopener noreferrer" class="social-button bg-[#1DA1F2] hover:bg-[#0c85d0]">
              <Icon name="mdi:twitter" class="w-5 h-5" />
           </a>
           <a :href="`https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`" target="_blank" rel="noopener noreferrer" class="social-button bg-[#1877F2] hover:bg-[#1158b6]">
              <Icon name="mdi:facebook" class="w-5 h-5" />
           </a>
           <a :href="`mailto:?subject=${encodedTitle}&body=Check out this article: ${encodedUrl}`" class="social-button bg-gray-500 hover:bg-gray-600">
