...
const forecasterBilly = async (scenarioId, save = false) => {
    // 1. GET SCENARIO
    if (!scenarioId) {
...
Please generate a forecast for the scenario.`;
    ...
    while (retryCount < maxRetries) {
        try {
            ...
            console.error(`Error during agent execution (attempt ${retryCount + 1}/${maxRetries}):`, error);
            retryCount++;
        }
    }
    throw new Error('Failed to get a valid response after 3 retries');
};
...
//   const result = await forecasterBilly(scenarioId, false);
//   console.log(result);
