...
const meta = {
    name: 'Strange-bot v0.1',
    description: 'gemini-2.5-pro based forecaster with exaSearch tool, instructions loosely based on Nate Silver\'s Superforecasting approach',
    type: 'AI',
    status: 'ACTIVE',
    modelDetails: {
        family: 'Gemini',
...
const invoke = async (scenarioId, save = false, log = false, existingForecast = null) => {
    ...
    const SYSTEM_PROMPT = generateSystemPrompt({
      roleDescription: 'You are a professional superforecaster with an amazing track record of accurate and well-calibrated probabilistic forecasts, and you have been tasked with forecasting the likelihood of an event occurring. Your tone and style is rationalistic and extremely analytical.',
      // For personaDescription, using the role's specific tone.
      personaDescription: 'Maintain a rationalistic and extremely analytical, yet objective, tone. Show your working by referencing your research approach and findings.',
...
export const forecasterStrange = {
    invoke,
    getName,
    getMeta,
};
...
//   const scenarioId = '681c1f33d173119b5f84b5d2'; // Replace with a valid scenario ID
//   try {
//     // const result = await forecasterStrange.invoke(scenarioId, false, true); // For initial forecast
//     // console.log('Initial Forecast Result:', JSON.stringify(result, null, 2));

//     // Simulate an existing forecast for update
//     const existing = {
//       _id: "someMongoIdStrange",
//       probability: 0.55,
//       rationalSummary: "Dr. Strange's initial calculation pointed to a balanced probability.",
//       rationalDetails: "Multiple timelines were observed, many leading to this outcome, many not...",
//       dossier: ["http://example.com/eye_of_agamotto_scan.log"],
//       timestamp: new Date(Date.now() - 10*24*60*60*1000).toISOString() // 10 days ago
//     };
//     const updateResult = await forecasterStrange.invoke(scenarioId, false, true, existing);
//     console.log('Update Forecast Result Strange:', JSON.stringify(updateResult, null, 2));

//   } catch (error) {
//     console.error('Error running Strange example:', error);
//   } finally {
